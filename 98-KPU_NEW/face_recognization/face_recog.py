...
ld5_kpu.load("/sd/KPU/face_recognization/ld5.kmodel")
    ...
    while 1:
        ...
        if len(dect) > 0:
            for l in dect :
                ...
                fea_kpu.run(feature_img)
                feature = fea_kpu.Feat.calculate(fea_kpu)
                scores = []
                for j in range(len(record_ftrs)):
                    score = fea_kpu.Feat.compare(record_ftrs[j], feature)
                    scores.append(score)
                if len(scores):
